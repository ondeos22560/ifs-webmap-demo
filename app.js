...
function renderKpis(arr){let u=uniqueProjects(arr);$('countBadge').textContent=`${u.length} projets`;$('kProjects').textContent=fmt(u.length);$('kBenef').textContent=fmt(u.reduce((s,p)=>s+p.beneficiaries,0));$('kOrgs').textContent=new Set(u.map(p=>p.org)).size;$('kPartners').textContent=new Set(u.map(p=>p.partner)).size;$('kFunders').textContent=new Set(u.map(p=>p.funder)).size;$('kAdmin2').textContent=new Set(arr.map(p=>p.admin2)).size;
